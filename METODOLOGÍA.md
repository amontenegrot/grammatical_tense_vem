...
* **Decisión Metodológica 2 (Estandarización Estricta y Z-score Independiente por Sesión):** Las medias y varianzas de las señales fMRI y de los espacios predictores se calcularon y aplicaron de manera completamente **independiente para cada historia (sesión)** antes de su concatenación. Además, se forzó la imputación nula de los vóxeles extra-corticales marcados como `NaN`.
* **Justificación 2:** Prevención de *Data Leakage* y control riguroso de la deriva del escáner (*Scanner Drift*). Dado que el escáner fMRI carece de un cero absoluto, la señal base biológica fluctúa significativamente entre sesiones. Concatenar las historias crudas genera "escalones" de varianza artificial que destruyen la precisión del modelo Ridge. El Z-scoring por sesión neutraliza estos artefactos, garantizando que el $\Delta R^2$ refleje una auténtica capacidad de generalización fuera de muestra.
...
* **Justificación:** A diferencia de una permutación aleatoria (shuffle) clásica, el desplazamiento circular destruye la asociación estímulo-corteza preservando la autocorrelación temporal inherente del flujo sanguíneo fMRI. La corrección Benjamini-Hochberg (Tasa de Falso Descubrimiento, FDR, $\alpha < 0.05$) restringe la aparición de falsos positivos derivados de realizar decenas de miles de contrastes simultáneos (uno por vóxel).

### 6.4. Interpretación Estadística y Consenso Grupal
* **Acción:** Automatización de la evaluación del Tamaño del Efecto ($\Delta R^2$) y Extensión Espacial a nivel de Sujeto Único (*Single-Subject*) y Muestral (*Group-Level*).
* **Justificación (Umbrales de Literatura):** En paradigmas naturalistas masivos, los valores nominales de $R^2$ son biológicamente menores debido al ruido no estacionario continuo. Se establecieron umbrales interpretativos fundamentados en la literatura (ej. Huth et al., 2016), donde un $\Delta R^2 \approx 0.01$ (1% de varianza única extraída por encima del ruido base) se considera un efecto focalizado válido, y un $\Delta R^2 \geq 0.05$ se tipifica como altamente robusto para un único rasgo.
* **Justificación (True Null Result):** El modelo fue diseñado matemática y metodológicamente para aceptar el **Resultado Nulo Verdadero**. Si el tiempo gramatical es altamente colineal con el procesamiento léxico-semántico y no aporta varianza *única* estadísticamente significativa frente a un modelo restringido, la corrección múltiple FDR forzará la representación a $0.0$, brindando una respuesta neurocientífica válida sobre el acoplamiento morfosintáctico-semántico sin inflar artificialmente la significancia (p-hacking).
* **Justificación (Consenso Descriptivo):** El reporte grupal se trata estrictamente como un "consenso cualitativo" de consistencia (ej. "hallado en X de Y sujetos") en lugar de una Inferencia Poblacional de Efectos Aleatorios (*Random Effects*), reconociendo las limitaciones estadísticas clásicas de los estudios fMRI naturalistas con muestras pequeñas e intensivas (*Deep Phenotyping*).
...
* **Decisión Metodológica 1 (Evadir límites de la API interna):** Se diseñó una integración de nivel de sistema para cargar las transformaciones anatómicas directamente desde el *Filestore* local (`.json`, `.ctm`), evadiendo restricciones dependientes de las versiones de la API de PyCortex y generando servidores web estáticos de libre acceso (WebGL/HTML).
* **Decisión Metodológica 2 (Arquitectura Visual de Doble Capa):** La proyección volumétrica interactiva se dividió en dos mapas intercambiables (*Layers*):
    * **Capa 1: *Global Language Network (R2 Global)***. Mapea la capacidad total del modelo frente a la corteza. Actúa como un *Sanity Check* fundamental, revelando áreas clásicas (ej. Giro Temporal Superior, Broca, Wernicke). Demuestra empíricamente que la adquisición fMRI es de alta fidelidad y el modelo computacional es altamente predictivo, sirviendo de ancla de validez para el resto de los hallazgos.
    * **Capa 2: *Tense Specific Network ($\Delta R^2$)***. Aísla espacialmente la red cerebral responsable del tiempo gramatical finito evaluado (el interrogante central del estudio), habiendo descontado ya la influencia semántica, sintáctica y acústica.
* **Decisión Metodológica 3 (Filtro Binario Estricto - Hard Thresholding):** Se descartó el uso de canales de opacidad gradual (*Alpha Masks*). En su lugar, a los vóxeles que no superaron el umbral estadístico (FDR > 0.05) se les imputó un valor `np.nan`.
* **Justificación:** Desde la perspectiva de software, esto previene colapsos de serialización JSON originados en los motores web de Python modernos. Desde la perspectiva científica, asegura un *Hard Threshold* estricto donde los vóxeles significativos conservan la intensidad y pureza total de su mapa de colores (unidimensional unidireccional `OrRd`), haciendo completamente invisible al ruido estadístico y revelando la anatomía de los surcos corticales subyacentes.